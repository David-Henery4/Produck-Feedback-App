...
import { filterFeedbackList, sortProductRequests } from "@/redux/features/prodReqsSlice";
    ...
    <div className="w-full bg-white p-6 flex flex-wrap justify-start items-center gap-2 rounded-[10px] max-w-[350px] lgTab:max-w-none lgTab:h-full">
      {filterOptions?.map((prod) => {
        return (
          <div
            key={prod?.id}
            className={`font-semibold text-[13px] py-[6px] px-4 rounded-[10px] capitalize mb-[6px] hover:cursor-pointer ${
              prod.isActive
